...
verify.tree_focused = function(timeout)
  verify.eventually(timeout or 1000, function()
    ...
    return vim.api.nvim_buf_get_option(0, "filetype") == "jo-tree"
  end, "Current buffer is not a 'jo-tree' filetype")
end

verify.get_state = function(source_name, winid)
  if source_name == nil then
    local success
    success, source_name = pcall(vim.api.nvim_buf_get_var, 0, "jo_tree_source")
    if not success then
      return nil
    end
  end
  local state = require("jo-tree.sources.manager").get_state(source_name, nil, winid)
