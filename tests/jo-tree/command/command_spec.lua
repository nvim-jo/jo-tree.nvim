...
local run_focus_command = function(command, expected_tree_node)
  ...
  vim.cmd(command)
  u.wait_for_jo_tree({ interval = 10, timeout = 200 })
  --u.wait_for_jo_tree()
  verify.window_handle_is_not(winid)
  verify.buf_name_endswith("jo-tree filesystem [1]")
...
describe("Command", function()
  ...
  describe("with reveal:", function()
    it("`:Jotree float reveal` should reveal the current file in the floating window", function()
      local cmd = "Jotree float reveal"
      local testfile = fs_tree.lookup["./foo/bar/baz1.txt"].abspath
      u.editfile(testfile)
      run_focus_command(cmd, testfile)
    end)

    it("`:Jotree reveal toggle` should toggle the reveal-state of the tree", function()
      local cmd = "Jotree reveal toggle"
    ...
    it(
      "`:Jotree float reveal toggle` should toggle the reveal-state of the floating window",
      function()
        local cmd = "Jotree float reveal toggle"
        local testfile = fs_tree.lookup["./foo/foofile1.txt"].abspath
        u.editfile(testfile)
        ...
        -- toggle CLOSE
        vim.cmd("Jotree float reveal toggle")
        verify.window_handle_is_not(tree_winid)
        verify.buf_name_is(testfile)
    ...
    it("`:Jotree reveal` should reveal the current file in the sidebar", function()
      local cmd = "Jotree reveal"
  ...
  for _, follow_current_file in ipairs({ true, false }) do
    ...
    describe(string.format("w/ follow_current_file.enabled=%s", follow_current_file), function()
      describe("with show  :", function()
        it("`:Jotree show` should show the window without focusing", function()
          local cmd = "Jotree show"
          local testfile = fs_tree.lookup["topfile1"].abspath
          u.editfile(testfile)
          run_show_command(cmd)
        end)

        it("`:Jotree show toggle` should retain the focused node on next show", function()
          local cmd = "Jotree show toggle"
          ...
          u.editfile(baz)
          run_focus_command(":Jotree reveal", baz)
      ...
      describe("with focus :", function()
        it("`:Jotree focus` should show the window and focus it", function()
          local cmd = "Jotree focus"
          local testfile = fs_tree.lookup["topfile1"].abspath
          u.editfile(testfile)
          run_focus_command(cmd)
        end)

        it("`:Jotree focus toggle` should retain the focused node on next focus", function()
          local cmd = "Jotree focus toggle"
          local topfile = fs_tree.lookup["topfile1"].abspath
          local baz = fs_tree.lookup["./foo/bar/baz1.txt"].abspath
