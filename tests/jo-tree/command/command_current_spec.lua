...
local run_in_current_command = function(command, expected_tree_node)
  ...
  verify.buf_name_endswith(string.format("jo-tree filesystem [%s]", winid), 1000)
...
describe("Command", function()
  ...
  describe("netrw style:", function()
    it("`:Jotree current` should show jo-tree in current window", function()
      local cmd = "Jotree current"
      run_in_current_command(cmd)
    end)

    it(
      "`:Jotree current reveal` should show jo-tree and reveal file in current window",
      function()
        local cmd = "Jotree current reveal"
    ...
    it("`:Jotree current reveal toggle` should toggle jo-tree in current window", function()
      local cmd = "Jotree current reveal toggle"
      local testfile = fs_tree.lookup["topfile1"].abspath
      u.editfile(testfile)
    ...
    it(
      "`:Jotree current reveal_force_cwd reveal_file=xyz` should reveal file current window if cwd is not a parent of file",
      function()
        vim.cmd("cd ~")
        local testfile = fs_tree.lookup["deepfile2"].abspath
        local cmd = "Jotree current reveal_force_cwd reveal_file=" .. testfile
        run_in_current_command(cmd, testfile)
      end
    )

    it(
      "`:Jotree current reveal_force_cwd reveal_file=xyz` should reveal file current window if cwd is a parent of file",
      function()
        local testfile = fs_tree.lookup["deepfile2"].abspath
