...
local jo_tree_preview_namespace = vim.api.nvim_create_namespace("jo_tree_preview")
...
local function create_floating_preview_window(state)
  ...
  local options = popups.popup_options("Jo-tree Preview", width, {
  ...
  options.zindex = 40
  options.buf_options.filetype = "jo-tree-preview"
...
--  is_jo_tree_window boolean Whether the preview window belongs to jo-tree.
...
function Preview:preview(bufnr, start_pos, end_pos)
  if self.is_jo_tree_window then
...
function Preview:revert()
  ...
  if self.config.use_float then
    ...
    vim.api.nvim_win_set_var(self.winid, "jo_tree_preview", 0)
  end
...
function Preview:findWindow(state)
  ...
  if self.config.use_float then
    ...
    winid = win.winid
    is_jo_tree_window = false
  else
    winid, is_jo_tree_window = utils.get_appropriate_window(state)
  ...
  self.winid, self.is_jo_tree_window = winid, is_jo_tree_window
...
function Preview:activate()
  ...
  self.active = true
  vim.api.nvim_win_set_var(self.winid, "jo_tree_preview", 1)
end
...
function Preview:clearHighlight()
  if type(self.bufnr) == "number" and vim.api.nvim_buf_is_valid(self.bufnr) then
    vim.api.nvim_buf_clear_namespace(self.bufnr, jo_tree_preview_namespace, 0, -1)
  end
end
    ...
    local preview_event = {
      ...
      handler = function()
        ...
        if vim.api.nvim_get_current_win() == winid then
          ...
          log.debug("Jo-tree window lost focus, disposing preview")
          Preview.hide()
        end
      end,
      id = "preview-event",
