...
local vim = vim
local fs_actions = require("jo-tree.sources.filesystem.lib.fs_actions")
...
local help = require("jo-tree.sources.common.help")
local Preview = require("jo-tree.sources.common.preview")
local async = require("plenary.async")
local node_expander = require("jo-tree.sources.common.node_expander")
...
local function get_folder_node(state)
  ...
  while node do
    ...
    local insert_as_global = require("jo-tree").config.window.insert_as
    local use_parent
...
local function get_using_root_directory(state)
  ...
  elseif show_path ~= nil and show_path ~= "none" then
    log.warn(
      'A jo-tree mapping was setup with a config.show_path option with invalid value: "'
        .. show_path
...
M.next_source = function(state)
  local sources = require("jo-tree").config.sources
...
M.order_by_created = function (state)
  ...
  require("jo-tree.sources.manager").refresh(state.name)
end
...
M.paste_from_clipboard = function(state, callback)
  if state.clipboard then
    ...
    paste_complete = function(source, destination)
      if callback then
        local insert_as = require("jo-tree").config.window.insert_as
