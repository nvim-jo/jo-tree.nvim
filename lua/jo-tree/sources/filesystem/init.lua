...
local git = require("jo-tree.git")
local glob = require("jo-tree.sources.filesystem.lib.globtopattern")
...
local follow_internal = function(callback, force_show, async)
  ...
  if vim.bo.filetype == "jo-tree" or vim.bo.filetype == "jo-tree-popup" then
    return false
  end
...
M.follow = function(callback, force_show)
  ...
  utils.debounce("jo-tree-follow", function()
...
M.reset_search = function(state, refresh, open_current_node)
  ...
  require("jo-tree.sources.filesystem.lib.filter_external").cancel()
  -- reset search state
...
M.setup = function(config, global_config)
  ...
  if config.use_libuv_file_watcher then
    ...
    require("jo-tree.sources.filesystem.lib.fs_watch").unwatch_all()
...
M.toggle_directory = function(state, node, path_to_reveal, skip_redraw, recursive, callback)
  ...
  elseif require("jo-tree").config.filesystem.scan_mode == "deep" then
