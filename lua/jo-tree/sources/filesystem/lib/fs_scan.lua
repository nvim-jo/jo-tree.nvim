...
local filter_external = require("jo-tree.sources.filesystem.lib.filter_external")
local file_items = require("jo-tree.sources.common.file-items")
...
local render_context = function(context)
  ...
  if not parent_id and state.use_libuv_file_watcher and state.enable_git_status then
    ...
    fs_watch.watch_git_index(path, require("jo-tree").config.git_status_async)
  end
...
local job_complete = function(context)
  ...
  local parent_id = context.parent_id
  if should_check_gitignore(context) then
    if require("jo-tree").config.git_status_async then
...
M.stop_watchers = function(state)
  if state.use_libuv_file_watcher and state.tree then
    ...
    fs_watch.unwatch_git_index(state.path, require("jo-tree").config.git_status_async)
