...
local fs_scan = require("jo-tree.sources.filesystem.lib.fs_scan")
local renderer = require("jo-tree.ui.renderer")
local inputs = require("jo-tree.ui.inputs")
local events = require("jo-tree.events")
local log = require("jo-tree.log")
local fs_watch = require("jo-tree.sources.filesystem.lib.fs_watch")
...
---Returns the state for the current buffer, assuming it is a jo-tree buffer.
...
M.get_state_for_window = function(winid)
  ...
  local source_status, source_name = pcall(vim.api.nvim_buf_get_var, bufnr, "jo_tree_source")
  local position_status, position = pcall(vim.api.nvim_buf_get_var, bufnr, "jo_tree_position")
...
M.navigate = function(state_or_source_name, path, path_to_reveal, callback, async)
  ...
  if not mod then
    mod = require("jo-tree.sources." .. source_name)
  end
...
--- use `require("jo-tree.command").execute({ source_name = source_name, action = "focus", reveal = true })` instead
M.reveal_current_file = function(source_name, callback, force_cwd)
  log.warn([[DEPRECATED: use `require("jo-tree.command").execute({ source_name = source_name, action = "focus", reveal = true })` instead]])
...
--- DEPRECATED: To be removed in 4.0
--- use `require("jo-tree.command").execute({ source_name = source_name, action = "focus", reveal = true, position = "current" })` instead
M.reveal_in_split = function(source_name, callback)
  log.warn([[DEPRECATED: use `require("jo-tree.command").execute({ source_name = source_name, action = "focus", reveal = true, position = "current" })` instead]])
...
M.setup = function(source_name, config, global_config, module)
  ...
  if module == nil then
    module = require("jo-tree.sources." .. source_name)
  end
