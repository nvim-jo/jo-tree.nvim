...
local command = require("jo-tree.command")
local M = {}
...
M.hijack = function()
  ...
  utils.debounce("hijack_netrw_" .. winid, function()
    ...
    local is_currently_jo_tree = false
    if replace_with_bufnr > 0 then
      if vim.api.nvim_buf_get_option(replace_with_bufnr, "filetype") == "jo-tree" then
        -- don't hijack the current window if it's already a Jo-tree sidebar
        local _, position = pcall(vim.api.nvim_buf_get_var, replace_with_bufnr, "jo_tree_position")
        if position ~= "current" then
          is_currently_jo_tree = true
        else
    ...
    local state
    if should_open_current and not is_currently_jo_tree then
      log.debug("hijack_netrw: opening current")
      state = manager.get_state("filesystem", nil, winid)
      state.current_position = "current"
    elseif is_currently_jo_tree then
      log.debug("hijack_netrw: opening in existing Jo-tree")
    ...
    require("jo-tree.sources.filesystem")._navigate_internal(state, bufname, nil, remove_dir_buf)
