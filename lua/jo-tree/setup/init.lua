...
local defaults = require("jo-tree.defaults")
local mapping_helper = require("jo-tree.setup.mapping-helper")
local events = require("jo-tree.events")
local log = require("jo-tree.log")
local file_nesting = require("jo-tree.sources.common.file-nesting")
local highlights = require("jo-tree.ui.highlights")
local manager = require("jo-tree.sources.manager")
local netrw = require("jo-tree.setup.netrw")
...
local define_events = function()
  ...
  events.subscribe({
    ...
    handler = function()
      require("jo-tree.ui.renderer").update_floating_window_layouts()
    end,
  })
end
...
local store_local_window_settings = function(winid)
  ...
  local jo_tree_settings_applied, _ =
    pcall(vim.api.nvim_win_get_var, winid, "jo_tree_settings_applied")
  if jo_tree_settings_applied then
...
local restore_local_window_settings = function(winid)
  ...
  if wo then
    ...
    vim.api.nvim_win_set_var(0, "jo_tree_settings_applied", false)
  else
...
M.buffer_enter_event = function()
  -- if it is a jo-tree window, just set local options
  if vim.bo.filetype == "jo-tree" then
    if last_buffer_enter_filetype == "jo-tree" then
      -- we've switched to another jo-tree window
      events.fire_event(events.JO_TREE_BUFFER_LEAVE)
    else
    ...
    local winhighlight =
      "Normal:JoTreeNormal,NormalNC:JoTreeNormalNC,SignColumn:JoTreeSignColumn,CursorLine:JoTreeCursorLine,FloatBorder:JoTreeFloatBorder,StatusLine:JoTreeStatusLine,StatusLineNC:JoTreeStatusLineNC,VertSplit:JoTreeVertSplit,EndOfBuffer:JoTreeEndOfBuffer"
    if vim.version().minor >= 7 then
      vim.cmd("setlocal winhighlight=" .. winhighlight .. ",WinSeparator:JoTreeWinSeparator")
    else
      vim.cmd("setlocal winhighlight=" .. winhighlight)
    end

    events.fire_event(events.JO_TREE_BUFFER_ENTER)
    last_buffer_enter_filetype = vim.bo.filetype
    vim.api.nvim_win_set_var(0, "jo_tree_settings_applied", true)
    return
  end

  if vim.bo.filetype == "jo-tree-popup" then
    vim.cmd([[
    setlocal winhighlight=Normal:JoTreeFloatNormal,FloatBorder:JoTreeFloatBorder
    setlocal nolist nospell nonumber norelativenumber
    ]])
    events.fire_event(events.JO_TREE_POPUP_BUFFER_ENTER)
    last_buffer_enter_filetype = vim.bo.filetype
    return
  end
  ...
  if last_buffer_enter_filetype == "jo-tree-popup" then
    events.fire_event(events.JO_TREE_POPUP_BUFFER_LEAVE)
  end
  last_buffer_enter_filetype = vim.bo.filetype
  ...
  if prior_type == "jo-tree" then
    local success, position = pcall(vim.api.nvim_buf_get_var, prior_buf, "jo_tree_position")
    if not success then
    ...
    local jo_tree_tabid = vim.api.nvim_buf_get_var(prior_buf, "jo_tree_tabid")
    if jo_tree_tabid ~= current_tabid then
      -- This a new tab, so the alternate being jo-tree doesn't matter.
      return
    end
    local jo_tree_winid = vim.api.nvim_buf_get_var(prior_buf, "jo_tree_winid")
    local current_winid = vim.api.nvim_get_current_win()
    if jo_tree_winid ~= current_winid then
      -- This is not the jo-tree window, so the alternate being jo-tree doesn't matter.
      return
    end
    ...
    vim.schedule(function()
      ...
      -- on options from the jo-tree window that are undesirable.
...
M.win_enter_event = function()
  ...
  -- if the new win is not a floating window, make sure all jo-tree floats are closed
  ...
  if M.config.close_if_last_window then
    ...
    if prior_exists and win_count == 1 and vim.o.filetype == "jo-tree" then
      local position = vim.api.nvim_buf_get_var(0, "jo_tree_position")
      local source = vim.api.nvim_buf_get_var(0, "jo_tree_source")
      if position ~= "current" then
        -- close_if_last_window just doesn't make sense for a split style
        log.trace("last window, closing")
        local state = require("jo-tree.sources.manager").get_state(source)
        if state == nil then
          return
        end
  ...
  if vim.o.filetype == "jo-tree" then
    local _, position = pcall(vim.api.nvim_buf_get_var, 0, "jo_tree_position")
    if position == "current" then
      -- make sure the buffer wasn't moved to a new window
      local jo_tree_winid = vim.api.nvim_buf_get_var(0, "jo_tree_winid")
      local current_winid = vim.api.nvim_get_current_win()
      local current_bufnr = vim.api.nvim_get_current_buf()
      if jo_tree_winid ~= current_winid then
        -- At this point we know that either the jo-tree window was split,
        -- or the jo-tree buffer is being shown in another window for some other reason.
        ...
        local old_state = manager.get_state("filesystem", nil, jo_tree_winid)
        vim.schedule(function()
          local bufnr = vim.api.nvim_get_current_buf()
          if bufnr ~= current_bufnr then
            -- The jo-tree buffer was replaced with something else, so we don't need to do anything.
            log.trace("jo-tree buffer replaced with something else - no further action required")
            return
          end
          ...
          local renderer = require("jo-tree.ui.renderer")
          state.force_open_folders = renderer.get_expanded_nodes(old_state.tree)
          require("jo-tree.sources.filesystem")._navigate_internal(state, nil, nil, nil, false)
        end)
        return
      end
    end
    -- it's a jo-tree window, ignore
    return
  end
...
M.merge_config = function(user_config, is_auto_config)
  ...
  local migrations = require("jo-tree.setup.deprecations").migrate(user_config)
  if #migrations > 0 then
    -- defer to make sure it is the last message printed
    vim.defer_fn(function()
      vim.cmd(
        "echohl WarningMsg | echo 'Some options have changed, please run `:Jotree migrations` to see the changes' | echohl NONE"
      )
    end, 50)
  end
  ...
  -- Prevent accidentally opening another file in the jo-tree window.
  events.subscribe({
    event = events.VIM_BUFFER_ENTER,
    handler = M.buffer_enter_event,
  })

  -- Setup autocmd for jo-tree BufLeave, to restore window settings.
  -- This is set to happen just before leaving the window.
  -- The patterns used should ensure it only runs in jo-tree windows where position = "current"
  local augroup = vim.api.nvim_create_augroup("JoTree_BufLeave", { clear = true })
  ...
  for _, source in ipairs(user_config.sources or default_config.sources) do
    ...
    if #parts == 1 then
      ...
      is_internal_ns, module = pcall(require, "jo-tree.sources." .. source)
    end
    if is_internal_ns then
      name = module.name or name
      all_sources[name] = "jo-tree.sources." .. name
    else
  ...
  require("jo-tree.command.parser").setup(all_source_names)
  ...
  if ( M.config.source_selector.winbar or M.config.source_selector.statusline )
    ...
    -- Removing this line makes Jo-tree show the "filesystem"
  ...
  for _, ss_source in ipairs(M.config.source_selector.sources or {}) do
    ...
    if source_match then
      ...
      log.debug(string.format("Unable to locate Jo-tree extension %s", ss_source.source))
    end
  end
  ...
  if M.config.auto_clean_after_session_restore then
    require("jo-tree.ui.renderer").clean_invalid_jotree_buffers(false)
    events.subscribe({
      event = events.VIM_AFTER_SESSION_LOAD,
      handler = function()
        require("jo-tree.ui.renderer").clean_invalid_jotree_buffers(true)
      end,
    })
  end
  ...
  require("jo-tree.ui.renderer").resize_timer_interval = rt
