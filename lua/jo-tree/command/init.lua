local parser = require("jo-tree.command.parser")
...
local completion = require("jo-tree.command.completion")
...
---Executes a Jo-tree action from outside of a Jo-tree window,
...
---                    "float",
---                    "current"
---  toggle = boolean  Whether to toggle the visibility of the Jo-tree window.
---  reveal = boolean  Whether to reveal the current file in the Jo-tree window.
...
M.execute = function(args)
  ...
  if args.source == "migrations" then
    require("jo-tree.setup.deprecations").show_migrations()
    return
  end
  ...
  -- If position=current was requested, but we are currently in a jo-tree window,
  -- then we need to override that.
  if args.position == "current" and vim.bo.filetype == "jo-tree" then
...
do_show_or_focus = function(args, state, force_navigate)
  ...
  if args.action == "show" then
    ...
    manager.navigate(state, args.dir, args.reveal_file, function()
      -- navigate changes the window to jo-tree, so just quickly hop back to the original window
