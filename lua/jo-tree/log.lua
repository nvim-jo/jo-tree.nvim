...
local notify = function(message, level_config)
  if type(vim.notify) == "table" then
    ...
    vim.notify(message, level_config.level, { title = "Jo-tree" })
  else
    local nameupper = level_config.name:upper()
    local console_string = string.format("[Jo-tree %s] %s", nameupper, message)
...
log.new = function(config, standalone)
  ...
  obj.use_file = function(file, quiet)
    if file == false then
      if not quiet then
        obj.info("[jo-tree] Logging to file disabled")
      end
      ...
      if not quiet then
        obj.info("[jo-tree] Logging to file: " .. obj.outfile)
      end
    end
  end
  ...
  local log_at_level = function(level, level_config, message_maker, ...)
    ...
    if config.use_file then
      ...
      if fp then
        ...
        print("[jo-tree] Could not open log file: " .. obj.outfile)
      end
    end
