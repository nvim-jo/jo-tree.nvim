local vim = vim
local q = require("jo-tree.events.queue")
...
local M = {
  ...
  JO_TREE_BUFFER_ENTER = "jo_tree_buffer_enter",
  JO_TREE_BUFFER_LEAVE = "jo_tree_buffer_leave",
  JO_TREE_LSP_UPDATE = "jo_tree_lsp_update",
  JO_TREE_POPUP_BUFFER_ENTER = "jo_tree_popup_buffer_enter",
  JO_TREE_POPUP_BUFFER_LEAVE = "jo_tree_popup_buffer_leave",
  JO_TREE_WINDOW_AFTER_CLOSE = "jo_tree_window_after_close",
  JO_TREE_WINDOW_AFTER_OPEN = "jo_tree_window_after_open",
  JO_TREE_WINDOW_BEFORE_CLOSE = "jo_tree_window_before_close",
  JO_TREE_WINDOW_BEFORE_OPEN = "jo_tree_window_before_open",
...
M.define_autocmd_event = function(event_name, autocmds, debounce_frequency, seed_fn, nested)
  local opts = {
    setup = function()
      local tpl =
        ":lua require('jo-tree.events').fire_event('%s', { afile = vim.fn.expand('<afile>') })"
      ...
      local cmds = {
        "augroup JoTreeEvent_" .. event_name,
    ...
    teardown = function()
      ...
      vim.cmd(string.format("autocmd! JoTreeEvent_%s", event_name))
    end,
