...
local filesize = require("jo-tree.utils.filesize.filesize")
...
---is a valid file or some sort of utility buffer like quickfix or jo-tree itself.
...
M.is_real_file = function(afile, true_for_terminals)
  ...
  local source = afile:match("^jo%-tree ([%l%-]+) %[%d+%]")
  if source then
    return false
  end
...
M.get_appropriate_window = function(state)
  ...
  if not suitable_window_found then
    -- go back to the jotree window, this will forve it to open a new split
    vim.api.nvim_set_current_win(current_window)
  end

  local winid = vim.api.nvim_get_current_win()
  local is_jo_tree_window = vim.bo.filetype == "jo-tree"
  vim.api.nvim_set_current_win(current_window)

  vim.o.eventignore = eventignore

  return winid, is_jo_tree_window
end
...
M.open_file = function(state, path, open_cmd, bufnr)
  ...
  if M.truthy(path) then
    ...
    if state.current_position == "current" then
      ...
      local winid, is_jo_tree_window = M.get_appropriate_window(state)
      vim.api.nvim_set_current_win(winid)
      -- TODO: make this configurable, see issue #43
      if is_jo_tree_window then
        local width = vim.api.nvim_win_get_width(0)
        if width == vim.o.columns then
          -- Jo-tree must be the only window, restore it's status as a sidebar
          width = M.get_value(state, "window.width", 40, false)
          width = M.resolve_width(width)
        end

        local split_command = "vsplit"
        -- respect window position in user config when Jo-tree is the only window
...
-- USAGE:
--   - `require("jo-tree.utils").brace_expand("x{a..e..2}")` -> `{ "xa", "xc", "xe" }`
--   - `require("jo-tree.utils").brace_expand("file.txt{,.bak}")` -> `{ "file.txt", "file.txt.bak" }`
--   - `require("jo-tree.utils").brace_expand("./{a,b}/{00..02}.lua")` -> `{ "./a/00.lua", "./a/01.lua", "./a/02.lua", "./b/00.lua", "./b/01.lua", "./b/02.lua" }`
