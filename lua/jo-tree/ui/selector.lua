...
local calc_click_id_from_source = function(winid, source_index)
  local base_number = #require("jo-tree").config.source_selector.sources + 1
...
-- https://github.com/nvim-jo-tree/jo-tree.nvim/pull/427#discussion_r924947766
...
---@param click_id integer: id passed to `___jotree_selector_click`, should be calculated with `M.calc_click_id_from_source`
---@return string: complete string to render one tab
local render_tab = function(left_sep, right_sep, sep_hl, text, tab_hl, click_id)
  local res = "%" .. click_id .. "@v:lua.___jotree_selector_click@"
...
M.get_scrolled_off_node_text = function(state)
  if state == nil then
    state = require("jo-tree.sources.manager").get_state_for_window()
    if state == nil then
      return
    end
  end
...
M.get = function()
  local state = require("jo-tree.sources.manager").get_state_for_window()
  if state == nil then
    return
  else
    local config = require("jo-tree").config
...
M.get_selector = function(state, width)
  ...
  return return_string .. "%<%0@v:lua.___jotree_selector_click@"
end
...
M.set_source_selector = function(state)
  local sel_config = utils.resolve_config_option(require("jo-tree").config, "source_selector", {})
  if sel_config and sel_config.winbar then
    vim.wo[state.winid].winbar = "%{%v:lua.require'jo-tree.ui.selector'.get()%}"
  end
  if sel_config and sel_config.statusline then
    vim.wo[state.winid].statusline = "%{%v:lua.require'jo-tree.ui.selector'.get()%}"
  end
end
...
_G.___jotree_selector_click = function(id, _, _, _)
  if id < 1 then
    return
  end
  local sources = require("jo-tree").config.source_selector.sources
  ...
  require("jo-tree.command").execute({
