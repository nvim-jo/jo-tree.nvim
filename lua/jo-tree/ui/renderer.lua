...
local windows = require("jo-tree.ui.windows")
...
---Clean up invalid jotree buffers (e.g after a session restore)
---@param force boolean if true, force cleanup. Otherwise only cleanup once
M.clean_invalid_jotree_buffers = function(force)
  ...
  for _, buf in ipairs(vim.api.nvim_list_bufs()) do
    ...
    local is_jotree_buffer = string.match(bufname, "jo%-tree [^ ]+ %[%d+]")
    local is_valid_jotree, _ = pcall(vim.api.nvim_buf_get_var, buf, "jo_tree_source")
    if is_jotree_buffer and not is_valid_jotree then
    ...
    if M.window_exists(state) then
      ...
      if bufnr == state.bufnr then
        ...
        if state.current_position == "current" then
          ...
          events.fire_event(events.JO_TREE_WINDOW_BEFORE_CLOSE, args)
          ...
          if focus_prior_window and #win_list > 1 then
            ...
            if current_winid == state.winid then
              local pwin = require("jo-tree").get_prior_window()
          ...
          events.fire_event(events.JO_TREE_WINDOW_AFTER_CLOSE, args)
        end
      end
    end
    state.winid = nil
...
M.render_component = function(component, item, state, remaining_width)
  ...
  if component_func then
    ...
    local name = component[1] or "[missing_name]"
    local msg = "Jo-tree: Component " .. name .. " not found."
    log.warn(msg)
    return { { text = msg, highlight = highlights.NORMAL } }
  end
end
...
local get_buffer = function(bufname, state)
  ...
  if bufnr < 1 then
    ...
    vim.api.nvim_buf_set_option(bufnr, "filetype", "jo-tree")
...
M.acquire_window = function(state)
  ...
  local bufname = string.format("jo-tree %s [%s]", state.name, state.id)
  ...
  events.fire_event(events.JO_TREE_WINDOW_BEFORE_OPEN, event_args)
  ...
  events.fire_event(events.JO_TREE_WINDOW_AFTER_OPEN, event_args)

  if type(state.bufnr) == "number" then
    vim.api.nvim_buf_set_var(state.bufnr, "jo_tree_source", state.name)
    vim.api.nvim_buf_set_var(state.bufnr, "jo_tree_tabnr", tabid_to_tabnr(state.tabid))
    vim.api.nvim_buf_set_var(state.bufnr, "jo_tree_tabid", state.tabid)
    vim.api.nvim_buf_set_var(state.bufnr, "jo_tree_position", state.current_position)
    vim.api.nvim_buf_set_var(state.bufnr, "jo_tree_winid", state.winid)
  end
  ...
  elseif position == "current" then
    ...
    if window_exists then
      ...
      if winbufnr < 1 then
        ...
        local success, buf_position = pcall(vim.api.nvim_buf_get_var, bufnr, "jo_tree_position")
        if not success then
          return false
        end
...
render_tree = function(state)
  local add_blank_line_at_top = require("jo-tree").config.add_blank_line_at_top
...
draw = function(nodes, state, parent_id)
  ...
  require("jo-tree.ui.selector").set_source_selector(state)
...
M.show_nodes = function(sourceItems, state, parentId, callback)
  ...
  if state.group_empty_dirs then
    if parent then
      local scan_mode = require("jo-tree").config.filesystem.scan_mode
